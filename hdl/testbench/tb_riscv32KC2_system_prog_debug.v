...
module tb_riscv32KC2_system;
    ...
    integer i;
    initial begin
	$readmemh("riscv32KC2_instr.verilog", instr_ram);
	for(i=0; i<4096;i=i+1) begin
		u_system.u_riscv_core.u_iram_sram.mem[i][07:00] = instr_ram[i*4 + 0];
		u_system.u_riscv_core.u_iram_sram.mem[i][15:08] = instr_ram[i*4 + 1];
		u_system.u_riscv_core.u_iram_sram.mem[i][23:16] = instr_ram[i*4 + 2];
		u_system.u_riscv_core.u_iram_sram.mem[i][31:24] = instr_ram[i*4 + 3];		 
	end
    end
    //----------------------------------------------------------------------------
    // Test Procedure
	...
	jtag_hart_clear_haltreq_013(20'd0);
        #2000000;
        jtag_write_DTM_013(32'h10,32'd3);//reset
